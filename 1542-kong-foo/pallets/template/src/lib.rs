...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(4)]
		#[pallet::weight(T::WeightInfo::store_unsigned_love())]
		pub fn store_unsigned_love(_origin: OriginFor<T>) -> DispatchResult {
			// Check that the extrinsic was signed and get the signer.
			// let who = ensure_signed(origin)?;
			...
			// Emit an event.
			Self::deposit_event(Event::SomethingUnsignedStored { something: love_message });
	...
	impl<T: Config> ValidateUnsigned for Pallet<T> {
		...
		fn validate_unsigned(
			...
			// // 检查调用是否为store_unsigned_love
			// match call {
			// 	Call::store_unsigned_love { .. } => {
			// 		// 这里可以添加更多的验证逻辑
			// 		ValidTransaction::with_tag_prefix("Template")
			// 			.priority(TransactionPriority::MAX)
			// 			.longevity(TransactionLongevity::MAX)
